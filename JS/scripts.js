...
// Butonların yüksekliğini eşitleme fonksiyonu
function equalizeButtonHeight() {
  const headerButtons = document.querySelectorAll('.header-button');
  if (headerButtons.length < 2) return;
  
  // Butonları normal yüksekliğe resetle
  headerButtons.forEach(btn => {
    btn.style.height = 'auto';
  });
  
  // En büyük yüksekliği bul
  let maxHeight = 0;
  headerButtons.forEach(btn => {
  ...
  // Tüm butonların yüksekliğini eşitle
  headerButtons.forEach(btn => {
...
$(document).ready(function() {
  ...
  // Buton yüksekliklerini eşitle
  equalizeButtonHeight();
});
